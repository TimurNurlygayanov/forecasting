...
RANK = {}
TAKE_PROFIT_THRESHOLD = 1.04  # 4 % of price increase
STOP_LOSSES_THRESHOLD = 0.97  # 2% risk
...
def run_backtest(ticker='AAPL', period='700d'):
    ...
    for i, (index, row) in enumerate(df.iterrows()):
        ...
        if last_buy_position == 0:
            if 0 > row['MACD'] > row['MACD_signal'] and df['MACD'].values[i-1] < df['MACD_signal'].values[i-1]:
                buy_signals[i] = True
                last_buy_position = i
