...
def run_me(ticker, progress_value) -> list:
    ...
    for i, (index, row) in enumerate(df_last_month.iterrows()):
        if len(df_last_month) - 5 > i > 5:
            if row['RSI'] > 13 > df_last_month['RSI'].values[i-1]:  # 12
                if row['Close'] < row['EMA7']:
                    buy_price = df_last_month['Open'].values[i + 1]
                    stop_loss = buy_price - row['Volatility'] / 3   #  min(df_last_month['Low'].values[i-3:i + 1]) - 0.05
