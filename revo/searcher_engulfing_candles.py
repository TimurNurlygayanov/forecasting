...
def search(ticker='AAPL', period='700d'):
    ...
    if df['S_trend_d'][-1] > 0:
        if df['Close'][-2] > df['Open'][-3] and df['Open'][-2] < df['Close'][-3]:
            if df['Open'][-3] > df['Close'][-3] and df['Open'][-2] < df['Close'][-2]:  # the previous candle should be red and new candle should be green
